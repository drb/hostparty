            ...
            if (options.path) {
                ...
                switch (os.platform()) {
                    ...
                    case 'win32':
                        ...
                        if (filePath) {
                            filePath = path.resolve(util.format('%s\%s', filePath, 'System32\drivers\etc\hosts'));
                        }
                        break;
                    default:
                }
                ...
                getHostFilePath()
                    .then(function(path) {
                        ...
                        let contents = [
                            util.format('# '),
                            util.format('#    /)'),
                            util.format('#  (/   ___ _  _/___   _   __ _/_)        v%s / %s', pkg.version, new Date()),
                            util.format('#  / )_(_) /_)_(__/_)_(_(_/ (_(__(_/_     %s', pkg.repository),
                            util.format('#              .-/              .-/'),
                            util.format('#             (_/              (_/'),
                            util.format('')
                        ];
                        ...
                        //
                        return fs
                            .writeFileAsync(path, util.format("%s%s", contents.join("\n"), "\n"))
                            .then(resolve)
                            .catch(function(e) {
